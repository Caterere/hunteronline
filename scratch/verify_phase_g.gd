...
func _ready() -> void:
	...
	if rel_sys != null:
		rel_sys.aplicar_escolha_social("wing", 0) # AJUDAR
		var rotulo = rel_sys.obter_rotulo_relacionamento("wing")
		print("  ✓ Rótulo social com Wing: %s" % rotulo)
	else:
		erros += 1

	var w_state = get_node_or_null("/root/WorldState")
	if w_state != null:
		w_state.registrar_treinamento_mestre("wing", "gyo")
		w_state.registrar_vitoria_boss("hisoka", "Arena Celestial")
		var diag = w_state.obter_dialogo_contextual_npc("wing")
	...
	if wsm != null:
		wsm.definir_clima(3) # TEMPESTADE_AURA
		var info = wsm.obter_estado_mundo_completo()
		var clima_info = info.get("clima", {})
		print("  ✓ Resumo do Mundo Vivo: Clima=%s, Hora=%d:%02d" % [clima_info.get("nome"), info.get("tempo", {}).get("hora", 8), info.get("tempo", {}).get("minuto", 0)])
	else:
		erros += 1
	...
	if dm != null:
		var eq_lamina = dm.obter_equipamento(&"lamina_cacador")
		if eq_lamina != null:
