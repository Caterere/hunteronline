...
func _ready() -> void:
	...
	if err_pack1 == OK:
		...
		print("[SUCCESS] estrada_padokia.tscn gerada com script valido! (code: %d)" % err_save1)
	else:
		printerr("[-] Erro ao empacotar estrada_padokia: ", err_pack1)
	estrada.free()
	...
	if err_pack2 == OK:
		...
		print("[SUCCESS] floresta_vestigios.tscn gerada com script valido! (code: %d)" % err_save2)
	else:
		printerr("[-] Erro ao empacotar floresta_vestigios: ", err_pack2)
	floresta.free()

	print("[*] Gerador concluido com sucesso!")
	get_tree().quit(0)