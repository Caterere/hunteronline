...
func _test_gps_no_story_gate_while_pending() -> void:
	print("-- GPS static --")
	var gsrc := FileAccess.get_file_as_string("res://ui/hud/MissionGPSIndicator.gd")
	assert_test("MissionObjectiveResolver.resolve" in gsrc, "GPS usa MissionObjectiveResolver")
	...
	assert_test("MissionObjectiveResolver.resolve" in qhud, "QuestHUD sincronizado com resolver")
