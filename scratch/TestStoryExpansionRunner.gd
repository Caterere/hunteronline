...
func _ready() -> void:
	print("=========================================================")
	print("[TEST] INICIANDO AUDITORIA E TESTE DE CONTEÚDO EXPANDIDO")
	print("=========================================================")
	...
	# --------------------------------------------------------
	# TESTE 6: Curva de Escalonamento de XP e Jenny
	# --------------------------------------------------------
	total_tests += 1
	var xp_arco1 := CanonQuestCatalog.obter_quest_da_etapa(1, 1).reward_xp
	var xp_arco5 := CanonQuestCatalog.obter_quest_da_etapa(5, 1).reward_xp
	var xp_arco9 := CanonQuestCatalog.obter_quest_da_etapa(9, 26).reward_xp
	var gold_arco1 := CanonQuestCatalog.obter_quest_da_etapa(1, 1).reward_gold
	var gold_arco9 := CanonQuestCatalog.obter_quest_da_etapa(9, 26).reward_gold
	
	var curve_ok := (xp_arco1 < xp_arco5 and xp_arco5 < xp_arco9 and gold_arco1 < gold_arco9)
	if curve_ok:
		print("✅ TESTE 6: Escalonamento de XP (%d -> %d -> %d) e Jenny (%d -> %d) suave e progressivo!" % [
			xp_arco1, xp_arco5, xp_arco9, gold_arco1, gold_arco9
		])
		passed_tests += 1
	else:
	...
	if passed_tests == total_tests:
		print("🏆 TODOS OS 6 TESTES DE AUDITORIA E EXPANSÃO PASSARAM COM 100% DE SUCESSO!")
	else:
