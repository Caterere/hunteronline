...
| **Zelda 2D** | • Exploração instigante ("Como chego lá?")<br>• Dungeons, atalhos, segredos e puzzles<br>• Interação física/lógica com o mapa | Exploração sem marcadores obsessivos; progressão através de novas capacidades e ferramentas de Nen. |
...
$$\text{Fraqueza/Condição (HxH + CrossCode)} + \text{Combate Preciso (HLD)} + \text{Exploração (Zelda)} + \text{Mundo/Progressão (Xenoverse)} = \text{Nosso Sistema}$$
...
2. **Ambiente / Exploração (Zelda / CrossCode)**: O jogador atrai o inimigo para fora de sua zona favorável ou usa o ambiente a seu favor.
...
3. **NEN** (Ten, Zetsu, Ren, Gyo, Shu, Ko)
4. **HATSU** (Sistemas modulares, condições, categorias)
5. **EXPLORAÇÃO** (Puzzles, segredos, utilidade de Nen no mapa)
...
9. **MUNDO** (Escala, viagens, rumores)
...
- Hatsu não possui dano plano estático; escala proporcionalmente aos atributos do usuário.

### 5.4 Balanceamento Baseado em TTK (Time-To-Kill)
