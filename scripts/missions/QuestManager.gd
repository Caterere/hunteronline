...
func fail_quest(quest: Quest) -> void:
	...
	if not PlayerData.is_quest_active(quest):
		return
	PlayerData.fail_quest(quest)
	active_quests.erase(quest)
	print("QUEST FALHOU: ", quest.quest_name)
	var hud_fail = get_tree().get_first_node_in_group("player_hud")
	if hud_fail != null and hud_fail.has_method("exibir_notificacao"):
		hud_fail.exibir_notificacao("💀 Missão Falhou: %s" % quest.quest_name)
